# ORCHESTRAI Content Creation Guide

## Overview

This guide documents the mandatory content creation workflow for ORCHESTRAI. This workflow applies the **Universal Agent Delegation Pattern** using specialized content agents.

**Agent Types Used:**
- `content-writer-specialist` - General content creation
- `healthcare-content-specialist` - Healthcare-specific content
- `technical-documentation-specialist` - Technical documentation
- `multi-language-content-adapter` - Multi-language content adaptation
...
**CRITICAL**: This phase applies the **Universal Agent Delegation Pattern**.

1. **TodoWrite Checkpoint**: Mark "Write full article" as IN_PROGRESS
2. **MANDATORY: Use Task Tool for Specialized Agents**: Never write articles directly with Write tool
   - For content: Use `content-writer-specialist`
   - For healthcare: Use `healthcare-content-specialist`
   - For technical: Use `technical-documentation-specialist`
   - **ANY specialized agent** via Task tool - not hardcoded to content-writer-specialist
3. **PARALLEL EXECUTION DEFAULT**: When writing 2+ independent articles, launch ALL agents simultaneously in ONE message (60-70% time savings)
   - Works across ALL domains: content, SEO, research, etc.
   - Example: Launch 3x content-writer-specialist in parallel, or mix content + SEO + client analysis agents
4. **AI DETECTION PREVENTION (MANDATORY)**: Include explicit anti-AI instructions in every agent prompt:
...
5. **Specialized Agent Instructions**: Provide comprehensive outline and requirements to agents
...
**See [CLAUDE.md](CLAUDE.md) for universal agent delegation patterns and system architecture.**
